# Dispatch Board — the complete build record

Everything used to build this platform: every instruction given, the rules that shaped every decision, the schema as it grew, and — most usefully for a second build — the bugs and what they cost.

Written for starting the next one. It assumes the reader is the person who built this, so nothing is generalised away: the specifics are the point. Live credentials are the only thing removed.

## What it came to

- **147** instructions given
- **30,063** words of instruction
...
These were said once and then applied to everything afterwards. On a new build, paste this section into the first prompt — it is the part that stops an agent inventing an integration, sending something nobody approved, or reporting a feature as finished because the code exists.
...
## 2. What went wrong, and what it taught

The most valuable section here. Every one of these was expensive to find and cheap to prevent, and none of them were obvious in advance.

### Written but never wired

The single most common failure, by a wide margin. Logic written correctly, tested in isolation, and never called by anything real.
...
**What to do instead:** Write a test that reads the PRODUCTION file and asserts it calls the thing. Then reintroduce the bug and confirm the test fails — twice this caught a guard that asserted a constant existed rather than that it was used, which is the same class of bug one level up.
...
**What to do instead:** After any write to a system you do not own, read it back and check the SHAPE, not just presence. 'Is it empty' was too weak a check and shipped the second bug.
...
**What to do instead:** Build one view that walks the whole chain in order and names only the FIRST break. Fixing stage four while stage one is dry is how a fortnight goes by.
...
**What to do instead:** Write the reason down on EVERY exit path, including the quiet ones, and especially 'it is switched off'. Put it where somebody looks, not in a log.
...
**What to do instead:** Test that every value the code can produce is accepted by the column. Select columns in TIERS so an unrun migration degrades the page instead of failing the whole query.
...
**What to do instead:** Say 'I do not know yet, here is the query that settles it'. Then run it.

---

## 3. The schema, in the order it grew

Each migration is a decision with a date on it. Read top to bottom and it is the shape of the problem being understood in real time.

- `0001_core` — Dispatch Board core schema
- `0002_events` — Append-only organizational memory.
- `0003_enrichment` — Enrichment & decision-maker discovery, folded into Dispatch Board.
- `0004_lead_stages` — Leads live on the sales board. Run after 0003_enrichment.sql.
- `0005_canonical_stages` — Canonical pipeline stages, required lead columns, and explicit RLS.
- `0006_engine_foundation` — Lead-generation engine: sourcing campaigns, search tasks, job queue.
- `0007_cron` — OPTIONAL: run the engine headlessly, without any browser tab open.
- `0008_packets_from_sourcing` — Let caller packets be generated from sourcing campaigns.
- `0009_auto_assign` — Campaigns can hand finished leads straight to callers with no manual step.
- `0010_multi_location` — Campaigns can target many metros at once, not just one city.
- `0011_owner_intel` — Owner-first calling: durable lead intelligence, structured outcomes,
- `0012_event_memory` — Organizational memory, Milestone 1.
- `0013_call_analytics` — Analytics capture layer.
- `0014_dnc_enforcement` — Do-not-call enforcement.
- `0015_callable_target` — Target callable leads, not saved businesses.
- `0016_caller_trials` — Caller tryouts.
- `0017_contacted_backfill` — Fix leads that were called but never marked as such.
- `0018_prompts` — Editable AI prompts.
- `0019_call_intelligence` — Call Intelligence.
- `0020_performance_targets` — Absolute targets, so "better than the team" can never masquerade as "good".
- `0021_browser_recordings` — Browser room recording.
- `0022_ai_authority` — The AI gets the last word, and stays checkable.
- `0023_owner_enrichment` — Owner-enriched leads with direct telephone numbers.
- `0024_workshop_packets` — Workshop packets, and the gatekeeper script test.
- `0025_team_updates` — Team updates.
- `0026_one_party_only_consent` — A consent policy that skips two-party states entirely.
- `0027_unstick_leads` — Give the callers their leads back.
- `0028_restore_uncalled_packet_leads` — Put back the leads that were closed without ever being called.
- `0029_instantly_email` — Instantly: cold email alongside the calling.
- `0030_email_autonomy` — Three things, all in service of the email programme running itself.
- `0031_sending_capacity` — Let the inboxes decide how much gets sent, and let them grow.
- `0032_diagnostic` — Somewhere to put a real diagnosis, and a size for the business.
- `0033_house_knowledge` — One place for what the house has learned, and a record of where it
- `0034_script_version_widen` — Let the gatekeeper test have more than three variants.
- `0035_schedule_worker` — Actually schedule the worker.
- `0036_email_audience` — Record WHO each email was aimed at, not just where the address came
- `0037_refill_note` — Write down what the automatic top-up decided.
- `0038_named_people_only` — Let an administrator refuse general inboxes.
- `0039_after_call_gate` — The after-call gate, switchable by the owner.
- `0040_auto_reenrich` — Let the re-enrichment backfill run itself, the same way the email top-up
- `0041_keep_the_funnel_full` — Keep the funnel full without anybody pressing anything.
- `0042_require_named_person` — "I need them to be personal emails with their name attached."

---

## 4. How the machine actually runs

One worker endpoint, called every minute by pg_cron. It claims jobs from a Postgres table with `FOR UPDATE SKIP LOCKED`, so two workers never take the same job, and every job carries an idempotency key so a retry is a no-op.

The chain, in order — and every stage can stop independently:
...
**The lesson that cost the most:** every stage reported on itself honestly and it was still impossible to tell where it had stopped. Build the whole-chain view on day one.

---

## 5. The build, commit by commit

105 commits, oldest first. The titles were written to say WHY.

- Dispatch Board: fresh rebuild — Kanban, PIN dialer, packets, metrics, scraper scripts, events memory, AI prioritizer
- Brown terminal restyle + Enrichment Desk Milestone 1
- Fold enrichment into Dispatch Board; remove separate app
- Sales board shows leads as stage cards
- Fix lead loading: canonical stages, surfaced errors, diagnostics
- M1+M2: async lead-generation engine (Google Places)
- Surface engine setup errors instead of claiming no campaign exists
- Make missing-env-var errors say to redeploy
- Teach the prioritizer what this system automates
- Make generated leads callable; broaden to all home services
- Add supabase/SETUP.sql: one-shot idempotent database setup
- Separate caller app from admin; make campaigns run themselves
- Reduce campaign creation to three questions and auto-start it
- One-button lead generation across a mix of trades and metros
- Explain Places 403s and missing caller secret instead of failing quietly
- Add a Places key test that shows Google's full response
- Recognize bare PERMISSION_DENIED as an inactive-billing signal
- Website enrichment waterfall + size cap on generated leads
- Desktop dialer layout and pluggable enrichment sources
- Show the deployed commit in the UI
- Show the call tip again instead of hiding it behind a toggle
- Owner-first dialer with structured outcome capture
- Move the AI call tip under Call Objective
- Lean Milestone 1: organizational memory foundation
- Fix 0012 migration failing with "syntax error at end of input"
- Analytics: capture the facts that decay, then gate every conclusion
- Make do-not-call actually suppress
- Rewrite the lead page around what an operator actually needs to know
- Target callable leads instead of businesses saved
- Make packets and lead batches operable
- Let packets be deleted, and bad leads be binned
- Put the admin console behind a passphrase
- Fix the destructive buttons doing nothing
- Add the read-out: say what the data shows, not just whether it is proven
- Caller profiles, strength-based packet routing, and self-returning callbacks
- Caller tryouts: a standard packet, a frozen bar, and an honest scorecard
- One definition of "free to hand out", and stop losing called leads
- Client relationship: the full record for a lead, and a read on it
- Time: derive the working day from the work, not from what people report
- Adaptive packets: order by the business's clock, not the list
- Editable prompts, so wording changes do not need a deploy
- Call Intelligence: data model and the five engines
- Call Intelligence phases 3 and 4: follow-up queue and the live assistant
- Judge callers against an absolute bar, not just each other
- Record calls through the laptop mic, with the handset on speaker
- Rebuild the call screen around the call
- Let the AI decide what happened, and keep it checkable
- Stop admin pages white-screening when an API reports a problem
- Read the column the learning table actually has
- Put the bar in the same row as the caller's number
- Owner enrichment: find the decision-maker and a number that reaches them
- Packet link, trial capture, and the A/B/C gatekeeper script test
- Make 0024 survive a truncated paste, and say when it did not
- Explain the two Twilio errors a new account actually hits
- Fix stretched tickboxes and a nav bar that pushed every page sideways
- Team updates: post once, every caller reads the same words
- Fix Copy link, swap the demo for recommendations, add a skip-two-party policy
- Give the callers their leads back: the grade orders a packet, it does not gate it
- The dialer said "All done" with 96 leads still in the packet
- Reopen a packet that was closed with leads still pending, and say so
- Restore packet leads that were closed without ever being called
- Packet building no longer depends on a migration that may not have run
- The Send button was disabled by a count that had failed, not by a lack of leads
- Two-party states cannot be recorded at all; one-party states record themselves
- Default lead runs now search one-party states only
- Connect Instantly: push leads, catch replies, draft answers
- Make the email programme run itself, and give it somebody to write to
- Let the inboxes set the pace, and raise their limits for you
- Diagnose the business properly, and stop letting callers pick the script
- Make the learning feed every tool instead of five silos
- Collapse the navigation from 21 sections to 16
- Put the packet in the emails, so a prospect has something to click
- Fix the review queue: an ambiguous embed refused the whole query
- Stop "no transcript" from filling the review queue
- Fix calls_script_version_check rejecting D through G
- Add CHECK_MIGRATIONS.sql: ask the database, not memory
- Schedule the worker — nothing has ever been calling it
- Add a button to run the worker by hand
- Stop discarding personal addresses; order the waterfall by who it reaches
- Make CHECK_MIGRATIONS answer "why is nothing happening"
- Add a re-enrich backfill for leads enriched before we could collect anything
- Fix the verification query in 0035: job_run_details has no jobname
- Add CHECK_CALLERS_READY: one pre-flight check before a shift
- Recording is mandatory where it is lawful, and the phone is gated on it
- Say which migration to run when the one-party option is refused
- Say, at the top of the Email page, whether email is actually running
- Distinguish an empty campaign from a broken one
- Make the automatic top-up say what it decided
- Push named people before general inboxes, and let them be skipped entirely
- Make the auto-refill actually refill, by counting what we own
- VA accountability: the after-call gate, as pure decision logic
- Wire the after-call gate into the production path, server-side
- Retry failed pushes — a refused push is not a push
- The top-up's quiet exits now write a note too
- Count the campaign's daily limit, which is the cap that was actually binding
- Add a shareable prompt book
- Let re-enrichment catch up on its own, the same way the email top-up does
- Read the campaign back after publishing, instead of trusting the 200
- Send the documented body format, and check which one actually stores
- Add a read-only query for who received a blank email
- Send real paragraphs, and put the findings in the email itself
- Stop pitching, stop assuming it is the phone, and let the workshop be read
- No, and here is the greeting bug that came out of checking
- Show why the top-up did nothing, which is what 0037 was for
- Keep the funnel full, and make the whole chain visible in one place

---

## 6. The standalone specification documents

Written as files and handed over whole rather than typed into a chat. For anything large this works considerably better — it can be edited before sending, and the agent gets the whole shape at once.
...
## 7. Every instruction, in order

All 147 of them, verbatim, typos included. Harness noise, interrupt markers and context re-dumps are the only things left out.

The short ones matter as much as the long ones. Most of this build was course-correction, and almost every correction is a case where what got built was technically what was asked for and still not what was wanted.

### 1. I'm continuing work on two related apps built in a previous session. H…
...
### 2. Build a production-ready lead enrichment and decision-maker discovery …
...
### 3. the enrichment is a seperate app? everything needs to be on the dispat…
...
### 6. The sales board is rendering correctly, but lead cards are not consist…
...
### 8. The empty database is expected because this is a new system. Do not im…
...
### 9. The Google Places API key is configured in Vercel as: GOOGLE_PLACES_AP…
...
### 10. okay there must be must an issue. i plugged it in, nothing popped up o…
...
### 12. ## Sales Ops Briefing — July 26, 2026 --- **The data is completely emp…
...
### 13. dude, i need it to give me the leads so i CAN call. also way is it jus…
...
### 14. ## Today's Situation: The Pipeline is Empty There is nothing to work w…
...
### 15. i just checked it and its way too much manual work. last one i could s…
...
### 16. why is there all these extra buttons? when i press generate campaign, …
...
### 17. thats still too much. i want a mix of different services so i can natu…
...
### 18. okay so the packet hasnt generated anything. the dialer also doesnt wo…
...
### 19. it looks like i already did it
...
### 20. Google Places key test ✕ Result: HTTP 403 Key: 39 chars, AIzaSy…QX9w ·…
...
### 21. i changed it? is this what needed to happen?
...
### 22. Google Places key test ✕ Result: HTTP 403 Key: 39 chars, AIzaSy…QX9w ·…
...
### 23. its asking for this and i have none of the organization stuff
...
### 24. shit i have no clue why that popped up but its workign
...
### 25. how much will 100 leads cost in googles cloud?
...
### 27. Sign-in is not configured on this deployment. An admin needs to add CA…
...
### 28. look at the call tip. it suggested me a company that just doesn't make…
...
### 30. for milestone 3 with the better lead testing. where it test state regi…
...
### 33. pdate the existing caller dialer for an AI receptionist cold-calling c…
...
### 34. can you put the ai call tip under the objective
...
### 35. We are adding two major systems to the existing sales platform: 1. Org…
...
### 36. Continue from the audit and implement Lean Milestone 1 now. Do not sto…
...
### 37. Error running SQL query ``` Failed to run sql query: ERROR: 42601: syn…
...
### 38. so give me a rundown of what the website has and can do when it comes …
...
### 39. i understand you cannot have optimized results after nothing but are t…
...
### 40. Build the Complete Memory, Analytics, Experimentation, and Optimizatio…
...
### 42. okay so what is now in the website that has to do with analytics and m…
...
### 44. okay so i dont really know how to go from this, idk if it should be si…
...
### 45. so instead of me having to ask for double, cant you just change it so …
...
### 48. okay so look at those three pictures. there isnt a way for me to edit …
...
### 49. i still cant delete packets. the one jack has still has the not up to …
...
### 50. could you change the website name from having landing at the end to ra…
...
### 51. the red buttons dont work. trash, bin it, delete. none of that actuall…
...
### 52. how do my callers login into the the dial site? https://v2-4-points.ve…
...
### 53. no but the vercel site isnt visible to people outside of my email
...
### 54. can people still get into the admin website even with the code on it?
...
### 55. okay before you do it, what will change? how will that chnage affect m…
...
### 58. hey so my callers called and it worked. the only thing is that there i…
...
### 59. can you also include the callers information. Thats the big one becaus…
...
### 60. also can you make a test feature where it will automatically generate …
...
### 63. the adding leads to packet is super janky and needs to be fixed. The l…
...
### 64. okay so a caller got a lead but it just has the number and name on it.…
...
### 65. Is there a way to accurately track time in the website? I pay on a sep…
...
### 66. Can we make the packets alot more adaptive. Like can leads be moved ar…
...
### 67. Please put a little prompt helper in the chat. Sometimes I need to twe…
...
### 68. Build a Call Intelligence add-on inside the existing sales-performance…
...
### 71. Build a Call Intelligence add-on inside the existing sales-performance…
...
### 72. Do you think having an outside baseline of numbers put in is best for …
...
### 74. Can the build now have ai record and remember calls?
...
### 75. Let’s take the low-cost MVP route for now. Build a browser-based room …
...
### 77. Redesign the dialer to be significantly less bloated and more focused …
...
### 78. So can the dialer record calls, remember, and make moves based off the…
...
### 79. Can you add everything in the close that gap part. fort the review, ma…
...
### 80. explain steps 2 and 3 better
...
### 84. ``` -- Browser room recording. -- -- The low-cost route to recorded ca…
...
### 86. this keeps popping up on admin
...
### 87. i ran 19 and 22 both multiple times
...
### 88. okay i really need the baseline target numbers right next to the calle…
...
### 92. Upgrade the existing lead-generation system to produce owner-enriched …
...
### 94. okay so what needs to be implemented to get direct numbers?
...
### 95. Build prompt: Packet + Trial + Script Testing (lean version) Context T…
...
### 96. it worked. now i want to get this twilio set up
...
### 97. okay so those two bites. explain in simpler terms
...
### 98. okay so thats out of the question for today calling. this is how the r…
...
### 103. Build prompt: Team Updates panel on the Dispatch Board Goal A single p…
...
### 104. explain what the seed it exactly part is and what you put in verbatim
...
### 105. oh so what is in the prompt is what u used verbatim?
...
### 106. what does this mean? i want a toggle that will record on party consent…
...
### 107. hey this is a major issue. i can generate any leads. the way i got lea…
...
### 108. There is 96 lwads but he says its all done
...
### 109. still not working, i redeployed
...
### 111. those 96 were 100 leads that i put in before the work day startred. Th…
...
### 112. its still showing the same thing. slow down and genuinely work throw i…
...
### 113. so is there anything i have to do or just redeploy
...
### 114. now his name doesnt pop up
...
### 116. no but i added an additional 200 leads and its not popping up. He cant…
...
### 117. why does this consent button exist if its only on for one party consen…
...
### 118. go into the leads, and make so when a two party state emerges, no reco…
...
### 119. why do u keep bringing up michigan. They should be around the us
...
### 120. okay, look at where im getting my leads from and then make it so most …
...
### 121. can you connect instantly pretty seamlessely to this build? Like use t…
...
### 125. Can you make it smarter. it should be able to draft up its own squence…
...
### 126. can i turn on smart filling where itll look at the max numbers each em…
...
### 131. Make a/b/c testing random lol. One lead they get script a, on lead b. …
...
### 132. Have the learning become a positive parasite. The more trust and thing…
...
### 133. Can you look over the different sections. I feel like some are basical…
...
### 134. please add the packet to the emails. so they can press on the link and…
...
### 135. okay so i havent made a sequence, i can make one and itll automaticall…
...
### 136. what does there was no transcript even mean
...
### 138. Did you change the way lead packets were made? It still feels ai recep…
...
### 140. this migration backlog, these are all packets i need to run?
...
### 141. my question is why? ive added all of these things before
...
### 142. so does it work? are the email leads being sent to DM emails and not c…
...
### 143. why does the personal pool collapse?
...
### 144. Please fix the email-scoring mistake first so we stop discarding perso…
...
### 145. Check over the dialer. is there anything thats holding them back?
...
### 146. i ran 00036 and nothing change. because ni change, check to see if 000…
...
### 147. yes i authorize the builds. Building this will now make the calling go…
...
### 149. okay i need to know if my callers can call trmw and im not going to be…
...
### 150. why is this still here? Asking if they agree. it only calls on one par…
...
### 151. the button is glitched and wont work
...
### 152. again i dont know why there is no sign or button or anything to let me…
...
### 153. Multiple issues, i had to manually put these in. they look like custom…
...
### 154. okay so why cant you just make it so it auto refills?
...
### 155. okay so i put 38 in and redployed. now can i sit back knowing emails a…
...
### 156. so its good, its not sending becuase its 9 at night
...
### 159. shouldnt there be more leads in by now?
...
### 160. why is only 4 emails being sent per email? why is there only 119 leads…
...
### 162. Take every word in here and put it into a file. Leave out anything sen…
...
### 163. <local-command-caveat>Caveat: The messages below were generated by the…
...
### 164. <command-name>/model</command-name> <command-message>model</command-me…
...
### 165. <local-command-stdout>Set model to claude-sonnet-5</local-command-stdo…
...
### 166. what do i have to do to get the leads pushing automatically?
...
### 167. why do i have to press another button for enrichment
...
### 168. The emails have nothing attached to them. i type out what oi wanted in…
...
### 169. okay so its working but only 100 were being sent out when i have the l…
...
### 170. Can you change it so its less salesy. also. I want to be able to see i…
...
### 171. can you confirm that each email gets the person name attached to it an…
...
### 172. why are more leads not being fed it. it stop because it through all th…
...
### 173. ran 40 and redeployed. slow down and take your time. I dont want to fi…
...
### 174. okay so im currently building out a deal dispatch where i use Vas. ple…
